...
class class_step(object):
    ...
    def add_printer(self, msg, userId):
        ...
        if not current_type:
            if not self.class_database_op.get_redis_kv(userId, 'inputing'):
                ...
                return '请选择您打印机的连接类型，1为Google Cloud Printer, 2为 Epson Connect Email.'
            else:
                ...
                if msg['Content'] == '1':
                    ...
                    self.class_database_op.set_redis_kv(userId, 'printer_current_type', msg['Content'])
                    return '现在，请输入您打印机的 email .'

        current_value = self.class_database_op.get_redis_kv(userId, 'printer_current_value')
        if not current_value:
            self.class_database_op.set_redis_kv(userId, 'printer_current_value', msg['Content'])
            return '现在，请输入您打印机的名称.'

        current_name = msg['Content']
    ...
    def print_job_add(self, msg, userId, msg_opt = None):
        ...
        amount = self.class_database_op.get_redis_kv(userId, 'current_print_amount')
        if not amount:
            self.class_database_op.delete_redis_kv(userId, 'inputing')
            ...
            if msg['Content'] == '0':
                ...
                self.class_database_op.set_redis_kv(userId, 'current_print_amount', amount)
        ...
        if not print_file_name or not print_page_count or not print_price or not create_time:
            ...
            itchatmp.send("每份页数：%d，共打印 %d 份，以下是打印预览。继续打印请转账 %.2f 元，不打印请回复0。" % (results[0], amount, print_price), msg['FromUserName'])
        ...
        if msg['Content'] == '0':
            ...
            itchatmp.send("收到转账：%.2f元。时间戳：%d" % (msg_opt['money'], msg_opt['create_time']), msg['FromUserName'])
        ...
        self.class_database_op.add_print_job(print_page_count, print_file_name, user, printer)
